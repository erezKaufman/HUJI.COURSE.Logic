...
UNARY_TERNARY_SYMBOL = '?:'

BINARY_TERNARY_SYMBOLs = ['?',':']
...
def generate_ternary(exp1,exp2,exp3):
...
def generate_tests_infix(num_of_tests, max_length):
    ...
    for i in range(num_of_tests):
        ...
        for j in range(exp_len):
            ...
            binary_symbols_index = random.randint(0, 5)
            if action is 1:
                exp = UNARY_SYMBOL + exp
            elif action is 2:
                exp = '(' + exp + BINARY_SYMBOLS[binary_symbols_index] + generate_var() + ')'
            elif action is 3:
                exp = '((' + exp + BINARY_SYMBOLS[binary_symbols_index] + generate_var() + ')'+ \
                      BINARY_SYMBOLS[(binary_symbols_index+1)%2] + '(' + generate_var() + BINARY_SYMBOLS[binary_symbols_index] +\
                      generate_var() + '))'
            else:
                exp1 = '((' + exp + BINARY_SYMBOLS[binary_symbols_index] + generate_var() + ')' + \
                      BINARY_SYMBOLS[(binary_symbols_index + 1) % 2] + '(' + generate_var() + BINARY_SYMBOLS[
                          binary_symbols_index] + \
                      generate_var() + '))'
                exp2 = '((' + exp + BINARY_SYMBOLS[binary_symbols_index] + generate_var() + ')' + \
                      BINARY_SYMBOLS[(binary_symbols_index + 1) % 2] + '(' + generate_var() + BINARY_SYMBOLS[
                          binary_symbols_index] + \
                      generate_var() + '))'
                exp3 = '((' + exp + BINARY_SYMBOLS[binary_symbols_index] + generate_var() + ')' + \
                      BINARY_SYMBOLS[(binary_symbols_index + 1) % 2] + '(' + generate_var() + BINARY_SYMBOLS[
                          binary_symbols_index] + \
                      generate_var() + '))'
                exp = generate_ternary(exp1,exp2,exp3)
        tests.append(exp)
    return tests

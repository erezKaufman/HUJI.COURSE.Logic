...
def replace_equality_with_SAME(formulae):
    def add_rules_for_two(x, y):
        rules = []
        ret.append('A'+ x + '[' + 'SAME(' + x +',' + x +')]') #reflexivity for x
        ret.append('A'+ x + 'SAME(' + y +',' + y +')]') # reflex for y
        ret.append('A'+ x + '[A'+y+'['+'SAME('+x+','+y+') -> (SAME('+y+','+x+'))]]') # symtry for x,y->y,x
        ret.append('A'+ x + '[A'+y+'['+'SAME('+y+','+x+') -> (SAME('+x+','+y+'))]]') # symtry for y,x->x,y
        ret.append('A'+ x + '[A'+y+'['+'SAME('+y+','+x+') -> (R('+x+')->(R('+y+')))]]') # unary
        ret.append('A'+ x + '[A'+y+'['+'SAME('+y+','+x+') -> (R('+x+')->(R('+y+')))]]') # binary
        return rules

    def add_rules_for_three(x,y,z):
        # trasitiviyu
        ret.append('A'+x+'[A'+y+'+[A+'+z +'['+'SAME('+x+','+y+')&(SAME('+y+','+z+')->(SAME('+x+','+z+'))]]')

    def add_rule_for_realtion(r_vars):
    ...
    def SAME_helper(formula):
        ...
        for index, char in enumerate(formula):
            ...
            elif is_relation(char):
                cur_index = index
                r_vars = set()
                while formula[cur_index] != ')':
                    if char.isalnum():
                        r_vars.add(char)
                        cur_index +=1
                ret.append(add_rule_for_realtion(r_vars))
                front_index = cur_index

            elif char == '=':  # we need to replace this
        ...
        for var_1 in vars:
            for var_2 in vars:
                ret.extend(add_rules_for_two(var_1, var_2))
                for var_3 in vars:
                    ret.extend(add_rules_for_three(var_1, var_2, var_3))
        ret.append(add_rule_for_everyone(vars))

...
class Formula:
    ...
    def free_variables_helper(self, free: set, non_free=set()):
        ...
        if is_constant(self.root):
            ...
            self.get_Term_frees(self.first, free, non_free)  # appends all first term's free var's
            self.get_Term_frees(self.second, free, non_free)  # appends all second term's free var's

        elif is_relation(self.root):
            for arg in self.arguments:
                self.get_Term_frees(arg, free, non_free)  # for each argument (which is var) append all term's free's
    ...
    def get_Term_frees(self, arg, free, non_free):
        """
        add to free set all the variables that doesn't appear in the non_free set
        """
